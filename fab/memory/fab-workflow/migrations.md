...
Handled by `_init_scaffold.sh` during structural bootstrap:
...
**Decision**: `_init_scaffold.sh` assigns `0.1.0` to existing projects (detected via `config.yaml` presence) so `/fab-update` applies all needed migrations from the beginning.
