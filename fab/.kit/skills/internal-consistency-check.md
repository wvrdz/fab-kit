...
### Agent 2: Memory ↔ Implementation Drift

Prompt:

> Audit consistency between memory (`fab/memory/`) and the implementation (directories: `{IMPL_PATHS}`).
>
> 1. Read `fab/memory/index.md` to understand the memory landscape
> 2. Read every file in `fab/memory/` recursively
> 3. Read every file in the implementation directories: `{IMPL_PATHS}`
> 4. Report inconsistencies in these categories:
>    - **Stale memory**: memory files describing behavior/features that no longer exist or work differently
>    - **Missing memory**: implemented features not covered by any memory file
>    - **Wrong paths/names**: memory files referencing files, commands, or concepts by outdated names
>    - **Contradicted behavior**: memory files claiming behavior X when implementation does Y
>    - **Orphaned memory**: memory files not referenced from any index or other file
>
> For each finding, cite the specific files and lines involved.

### Agent 3: Specs ↔ Memory Drift

Prompt:

> Audit consistency between specs (`fab/specs/`) and memory (`fab/memory/`).
>
...
>    - **Terminology drift**: same concept described with different names across specs and memory
>    - **Coverage gaps**: spec concepts not reflected in memory, or memory topics not grounded in specs
>    - **Contradictions**: where memory and specs disagree on workflow, stages, behavior, or structure
>    - **Stale cross-references**: either layer referencing the other with outdated paths, names, or structure
>    - **Glossary drift**: terms defined in `fab/specs/glossary.md` that are used inconsistently in memory
>
> For each finding, cite the specific files and lines involved.
...
- **Add**: missing memory or spec coverage
...
- Memory files instruct users to do something that will fail or produce wrong results
- A referenced file, command, or path does not exist

A finding is **Minor** if:
- Naming mismatch without behavioral impact
- Coverage gap (missing memory, not wrong memory)
