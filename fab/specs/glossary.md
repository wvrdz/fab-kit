...
| **Memory files** | The source-of-truth documentation in `fab/memory/`. Organized by domain, updated by `/fab-continue` (hydrate) and `/docs-hydrate-memory` ingestion. Contrast with **spec.md** (change-scoped). |
...
| `/docs-hydrate-memory` | Ingests external documentation (Notion, Linear, local files) into `fab/memory/` with domain mapping. Dual-mode: ingest from sources or generate from codebase analysis. |
