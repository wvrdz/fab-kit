...
| **Centralized docs** | The source-of-truth documentation in `fab/memory/`. Organized by domain, updated by `/fab-continue` (archive) hydration and `/fab-hydrate` ingestion. Contrast with **spec.md** (change-scoped). |
...
| **Domain** | A topic area in `fab/memory/` (e.g., `auth/`, `payments/`). Each domain has its own `index.md` and one or more doc files. |
...
| **Design specs** | Human-curated, pre-implementation design documents in `fab/specs/`. Capture architectural intent, design rationale, and the "why" behind the system. Not managed by Fab tooling — organized however makes sense for the project. Contrast with **centralized docs** (post-implementation, AI-maintained via hydration). |
...
| `fab/memory/` | Centralized documentation. Organized by domain with index files. The authoritative post-implementation source of truth. |
