...
### Separate Skills Per Concern
Each command is its own skill file (`fab-init-constitution.md`, `fab-init-config.md`, `fab-init-validate.md`). This follows the Fab pattern of one skill = one action, with independent `model_tier` and frontmatter per skill.
