...
`/fab-backfill` detects structural gaps between `fab/docs/` and `fab/specs/` — topics that docs cover but specs don't mention at all — and proposes concise additions back to specs with interactive per-gap confirmation.
...
The skill operates on project-level `fab/docs/` and `fab/specs/` directories. It does not require `fab/current`, does not modify `.status.yaml`, and does not create git branches.

### Requirement: Pre-flight Checks

The skill SHALL verify `fab/docs/index.md` and `fab/specs/index.md` exist before proceeding. Missing indexes abort with guidance to run `/fab-init`.
...
| Change | Date | Summary |
|--------|------|---------|
| 260209-h3v7-fab-backfill | 2026-02-09 | Initial creation — `/fab-backfill` skill for detecting and backfilling structural gaps from docs to specs |
