...
| Hook registration | `fab-sync.sh` via `5-sync-hooks.sh` | Registers `fab/.kit/hooks/on-*.sh` into `.claude/settings.local.json` hooks; idempotent jq merge. Supports PostToolUse matchers (e.g., `on-artifact-write.sh` → Write + Edit entries) and matcher-less events (Stop, SessionStart) |
...
| Change | Date | Summary |
|--------|------|---------|
| 260306-6bba-redesign-hooks-strategy | 2026-03-06 | Updated hook registration row in delegation table to reflect PostToolUse matchers (Write/Edit for `on-artifact-write.sh`) and matcher-less events (Stop, SessionStart). |
