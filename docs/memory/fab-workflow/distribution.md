...
After extraction, the user MUST run `fab/.kit/scripts/lib/sync-workspace.sh` to create directories (`changes/`, `memory/`, `specs/`), skeleton files (copied from `scaffold/memory-index.md` and `scaffold/specs-index.md`), symlinks, `.envrc` (symlink to `scaffold/envrc`), and `.gitignore` entries (from `scaffold/gitignore-entries`). The bootstrap only provides `.kit/` — no `config.yaml`, `constitution.md`, or other project files.

**Scenarios**:
- Bootstrap a new project (no `fab/` directory) — creates `fab/.kit/` with all skills, templates, scripts, and VERSION file; running `lib/sync-workspace.sh` then creates `changes/`, `memory/index.md`, `specs/index.md`, symlinks, `.envrc`, and `.gitignore` entry
...
`fab/.kit/scripts/fab-upgrade.sh` SHALL download the latest `kit.tar.gz` from GitHub Releases, extract it to replace the current `fab/.kit/` contents, display the version change, and re-run `lib/sync-workspace.sh` to repair symlinks.

**Scenarios**:
- Update to a newer version — replaces `.kit/` contents, displays version change (e.g., "0.1.0 → 0.2.0"), re-runs `lib/sync-workspace.sh`, checks for version drift and prints `/fab-update` reminder if needed, preserves all files outside `.kit/`
...
After extracting the new `.kit/` contents, `fab-upgrade.sh` SHALL re-run `lib/sync-workspace.sh` to ensure all agent symlinks (`.claude/skills/`, `.opencode/commands/`, `.agents/skills/`) point to the updated skill files.
...
| 260213-3njv-scaffold-dir | 2026-02-13 | Updated bootstrap description to mention `lib/sync-workspace.sh` reads from `scaffold/` files for index templates, envrc, and gitignore entries |
| 260214-q7f2-reorganize-src | 2026-02-14 | Renamed `_init_scaffold.sh` → `lib/sync-workspace.sh` throughout; moved `fab-release.sh` from `fab/.kit/scripts/` to `src/scripts/` (dev-only, not shipped in kit) |
...
| 260212-emcb-clarify-fab-setup | 2026-02-12 | Updated bootstrap description to include `docs/specs/` directory and `design/index.md` in `lib/sync-workspace.sh` output |
