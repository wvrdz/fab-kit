...
| Change | Date | Summary |
|--------|------|---------|
| 260328-3tds-fix-wt-open-here-cd | 2026-03-28 | Added `wt shell-setup` subcommand for shell integration — outputs a wrapper function for bash/zsh that enables "Open here" cd (captures last stdout line, evals if it starts with `cd `). Wrapper sets `WT_WRAPPER=1` env var before invoking `command wt`; when unset, binary prints stderr hint (`hint: run "wt shell-setup" for setup instructions`). Updated `wt --help` to reference `eval "$(wt shell-setup)"` instead of showing raw wrapper code. |
