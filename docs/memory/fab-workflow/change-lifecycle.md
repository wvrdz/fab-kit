...
| Change | Date | Summary |
|--------|------|---------|
| 260305-02ip-archive-date-buckets | 2026-03-05 | Archive destination changed from flat `archive/{name}/` to date-bucketed `archive/yyyy/mm/{name}/`. Date derived from folder name's `YYMMDD` prefix (year prefixed with `20`). `archiveman.sh` gains `migrate` subcommand for one-time restructuring of existing flat entries (idempotent). `resolve_archive`, `cmd_list`, `backfill_index` updated to scan both flat and nested structures. `changeman.sh list --archive` updated for nested archive traversal. `fab-archive.md` skill doc updated with bucketed path references. |
