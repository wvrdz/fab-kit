...
**Copilot review fix**: `/git-pr-fix` is a standalone autonomous skill that waits for GitHub Copilot review comments on the current branch's PR, triages them (actionable vs informational), and auto-fixes actionable ones. Detection uses the GitHub reviews API polling for `copilot-pull-request-reviewer[bot]`. Two modes: **standalone** (first-poll bail — checks once, bails if no review) and **wait mode** (polls every 30s, max 12 attempts / 6 minutes — used when auto-invoked from `/git-pr` Step 6 where the PR was just created). Fixes are committed in a single `fix: address copilot review feedback` commit. The skill is idempotent — re-running after fixes finds no new modifications and exits cleanly. `/git-pr` auto-invokes this behavior as a best-effort Step 6 after printing "Shipped." — failures never block the shipping workflow.
