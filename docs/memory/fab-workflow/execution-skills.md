...
**Status mutations**: All `.status.yaml` progress transitions use `fab/.kit/bin/fab status` CLI event commands (`start`, `advance`, `finish`, `reset`, `fail`, `skip`) via the Bash tool, rather than direct file editing. Checklist metadata updates (`set-checklist generated`, `set-checklist total`, `set-checklist completed`) and confidence writes (`set-confidence`) are handled automatically by the `on-artifact-write.sh` PostToolUse hook when artifacts are written or edited — skills do not call these explicitly. The `skip` event transitions `{pending,active} → skipped` with forward cascade (all downstream pending stages become skipped). Skipped stages are treated as resolved for progression (like `done`). Resetting a skipped stage follows normal reset mechanics (`skipped → active`, downstream cascade to `pending`). This centralizes validation and ensures atomic writes with `last_updated` refresh. The `driver` parameter is optional but skills always pass it. Stage metrics (started_at, completed_at, driver, iterations) are updated automatically as side-effects.
...
| Change | Date | Summary |
|--------|------|---------|
| 260306-6bba-redesign-hooks-strategy | 2026-03-06 | Checklist metadata updates (`generated`, `total`, `completed`) and confidence writes are now hook-driven via `on-artifact-write.sh` PostToolUse hook. Updated Status mutations overview to separate hook-driven bookkeeping (automatic) from skill-driven stage transitions (explicit). |
