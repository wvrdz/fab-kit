...
**Decision**: `/git-review` is a standalone skill that handles all PR review types (human, Copilot, other bots), replacing `/git-pr-fix` (Copilot-only) and `/git-pr` Step 6 (inline auto-fix). Human reviews take priority — if any reviewer has commented, Copilot is not requested.
