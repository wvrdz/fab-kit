...
Agent runtime state lives in `.fab-runtime.yaml` at the repository root (gitignored). This file is NOT part of the workflow schema, NOT initialized by templates, and NOT read by statusman or any workflow script. It is managed by Claude Code hook scripts in `fab/.kit/hooks/` via `fab runtime` CLI subcommands: `fab runtime set-idle <change>` (writes `agent.idle_since` timestamp) and `fab runtime clear-idle <change>` (deletes the `agent` block). Both subcommands use the standard `<change>` argument convention (4-char ID, substring, full folder name) via the `resolve` package. The Go binary handles all YAML operations using `gopkg.in/yaml.v3` — no yq dependency.
...
Each worktree has its own repo root, so each gets its own `.fab-runtime.yaml` — no cross-worktree contention. The file is created on first write by `fab runtime set-idle`. External tools (e.g., pipeline orchestrator) can read this file to detect agent idle state without relying on timing heuristics.
...
| Change | Date | Summary |
|--------|------|---------|
| 260306-6bba-redesign-hooks-strategy | 2026-03-06 | Updated Ephemeral Runtime State: `.fab-runtime.yaml` now managed via `fab runtime set-idle` and `fab runtime clear-idle` Go subcommands (replaces yq). Standard change resolution via `resolve` package. |
