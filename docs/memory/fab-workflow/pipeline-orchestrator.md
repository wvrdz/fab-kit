...
Ship completion is detected by polling for a `.shipped` sentinel file at `$wt_path/fab/changes/$resolved_id/.shipped`. This file is written by `/git-pr` as its very last action, after all git operations (including a second commit+push of `.status.yaml` with the shipped URL) are complete. The sentinel replaces the previous `stageman is-shipped` polling approach, which had a TOCTOU race: `.status.yaml` was updated before the commit+push of that update completed, so the orchestrator could see "shipped" while the branch was still dirty. The sentinel file is gitignored and provides a race-free filesystem signal — its existence guarantees the branch tip is clean and the PR is fully created.
