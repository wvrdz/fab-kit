...
- Creates `fab/VERSION` (local project version — via `lib/sync-workspace.sh`)
...
- Creates skill symlinks via `lib/sync-workspace.sh` glob pattern
...
After extraction, run `fab/.kit/scripts/lib/sync-workspace.sh` then `/fab-init` as usual.
...
`/fab-init` delegates structural setup to `fab/.kit/scripts/lib/sync-workspace.sh` and adds interactive configuration on top. This means `lib/sync-workspace.sh` can be run independently (e.g., in CI or after a bootstrap download) without requiring `/fab-init`.

| Responsibility | Owner | Notes |
|---|---|---|
| Directories (`changes/`, `memory/`, `specs/`) | `lib/sync-workspace.sh` | Non-interactive, scriptable |
| `fab/VERSION` | `lib/sync-workspace.sh` | New project → engine version; existing project (has `config.yaml`) → `0.1.0`; existing file → preserved |
| Skeleton files (`memory/index.md`, `specs/index.md`) | `lib/sync-workspace.sh` | Copies from `scaffold/memory-index.md` and `scaffold/specs-index.md`; idempotent — skips if file exists |
| Skill symlinks (Claude Code, OpenCode, Codex) | `lib/sync-workspace.sh` | Discovers skills via glob pattern |
| `.envrc` symlink | `lib/sync-workspace.sh` | Links to `fab/.kit/scaffold/envrc` |
| `.gitignore` entries | `lib/sync-workspace.sh` | Appends entries from `scaffold/gitignore-entries` if not present |
...
`/fab-init` invokes `lib/sync-workspace.sh` as step 1g of its bootstrap sequence. Steps 1c–1f in `/fab-init` have idempotent guards so they gracefully skip artifacts already created by `lib/sync-workspace.sh`.

**Bootstrap path** (without `/fab-init`): After downloading `fab/.kit/` via curl or `cp -r`, running `lib/sync-workspace.sh` alone creates a complete structural scaffold. `/fab-init` is only needed to generate `config.yaml` and `constitution.md`.
...
| Change | Date | Summary |
|--------|------|---------|
| 260214-m3v8-relocate-docs-dev-scripts | 2026-02-14 | Updated `lib/sync-workspace.sh` delegation to create `docs/memory/` and `docs/specs/` instead of `fab/memory/` and `fab/specs/` |
| 260214-q7f2-reorganize-src | 2026-02-14 | Renamed `_init_scaffold.sh` → `lib/sync-workspace.sh` throughout (directory structure, delegation table, bootstrap references, design decisions, changelog entries) |
...
| 260213-iq2l-rename-setup-scripts | 2026-02-13 | Renamed `fab-setup.sh` → `lib/sync-workspace.sh` in delegation pattern and all references |
...
| 260212-emcb-clarify-fab-setup | 2026-02-12 | Added Delegation Pattern section documenting responsibility split between `/fab-init` and `lib/sync-workspace.sh` |
