...
- **`switch <name> | --blank`** — Switches the active change. Composes: resolve name via internal `resolve` logic, write `fab/current`, derive stage via `$STAGEMAN current-stage`, output structured summary. `--blank` deactivates by deleting `fab/current`. Called by `/fab-switch`.
