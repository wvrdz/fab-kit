...
- `fab pane` — parent command grouping all pane-related subcommands. Available from any directory, including outside a fab repo (sole entry in the router's `fabGoNoConfigArgs` allowlist — see Router section). Invoked with no subcommand, displays help listing the four subcommands below:
